...
class WandbLogger(LightningLoggerBase):
    ...
    def __init__(self,
        ...
        self._wandb_init = dict(name=name,
                                project=project,
                                id=version or id,
                                dir=save_dir,
                                resume=resume,
                                anonymous=anonymous_lut.get(
                                    anonymous, anonymous))
    ...
    def experiment(self) -> Run:
        ...
        if self._experiment is None:
            ...
            self._experiment = wandb.init(
                **self._wandb_init) if wandb.run is None else wandb.run
        ...
        for t, p, s in checkpoints:
            metadata = {
                ...
                'ModelCheckpoint': {
                    ...
                    for k in [
                        'monitor', 'mode', 'save_last', 'save_top_k',
                        'save_weights_only', '_every_n_train_steps',
                        '_every_n_val_epochs'
                    ]
